...
class Term_List_Endpoint {
	...
	/**
     * Sort terms by priorities
     *
     * @param Array of unsorted term's objects
     ...
     * @author David Tishbi
     */
	private function sortTags( $unSortedTerms, $tax_name ) {
        ...
        foreach ($tags_terms as $term) {
            ...
            $tags_term_data[$term->slug] = $priority ?: '0'; // Use '0' if not set -- Term meta data accept only native numbers {1, 2, 3, ...} (By definition)
        }

		usort($unSortedTerms, function ($a, $b) use ($tags_term_data) {
			$priorityA = isset($tags_term_data[$a['id']]) ? (int)$tags_term_data[$a['id']] : 0;
			$priorityB = isset($tags_term_data[$b['id']]) ? (int)$tags_term_data[$b['id']] : 0;
	...
	public function handle() {
		...
		try {
			...
			/**
			 * Use sorting for tags and regions taxonomies
			 * *New Conditions*
			 */
			if( $taxonomy->name === "case27_job_listing_tags" ) {
				$results = $this->sortTags( $results, 'case27_job_listing_tags' );
			}
			else if( $taxonomy->name === "region" ) {
				$results = $this->sortTags( $results, 'region' );
			}
